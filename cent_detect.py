...
def flush_detector(file_number, folder_name):
    ...
    ax.plot(times_plot, cent.T, label='Spectral centroid', color='w')
    ax.vlines(times_plot[detect_timecentroid], 0, 3, label='inicio e fim', color='green')
    ax.hlines(cent.T[detect_timecentroid[:, 0]].flatten(), times_plot[detect_timecentroid[:, 0]], times_plot[detect_timecentroid[:, 1]], label='limiar', color='green')
    ...
    axs.plot(time, cent.T, label="centroide", color="red")
    axs.vlines(time[detectSimples], 0, 1, linewidth=1, color='green')
    axs.vlines(times_plot[detect_timecentroid], 0, 3, label='inicio e fim', color='green')
    axs.hlines(cent.T[detectSimples[:, 0]].flatten(), time[detectSimples[:, 0]], time[detectSimples[:, 1]], label='limiar', color='green')
    ...
    for i in range(0, len(detectSimples), n_per_fig):
        ...
        for j, ax in enumerate(axs.flat):
            ...
            if idx < len(detectSimples):
                ...
                ax.vlines(time[detectSimples[idx]], 0, 1, label='inicio e fim', color='red')
                ax.hlines(norm_signal[detectSimples[idx][0]], time[detectSimples[idx][0]], time[detectSimples[idx][1]], label='inicio e fim', color='red')
