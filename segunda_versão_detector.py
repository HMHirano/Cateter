...
def flush_detector(file_number, folder_name):
    ...
    # Oscilações abaixo do mínimo
    n = 0 
    for ele in step:
        print(f"Verficação entre {ele[0]} e {ele[1]} ")
        #print(signal_raw_filtered[ele[0]:ele[1]])
        print(f"Valor mínimo: {min(signal_raw_filtered[ele[0]:ele[1]])}")
        fig, axs = plt.subplots(1,1)
        axs.plot(time[ele[0]:ele[1]],signal_raw_filtered[ele[0]:ele[1]])
        axs.set_title(f"Step {n} entre {ele[0]} e {ele[1]}")
        fig.show()
        n += 1
        if (signal_raw_filtered[ele[0]:ele[1]] < 0.3).any() :
            print(f"Step {n} entre {ele[0]} e {ele[1]} removido")
            step.remove(ele)
    ...
    for i in range(0, len(step), n_per_fig):
        ...
        for j, ax in enumerate(axs.flat):
            idx = i + j
            if idx < len(step):
                start_pos = max(0, step[idx][0] - 1*SAMPLE_RATE)
                end_pos = min(len(signal_raw_filtered), step[idx][1] + 10*SAMPLE_RATE)
                ax.plot(time[start_pos:end_pos], signal_raw_filtered[start_pos:end_pos], label=f'Instant of flush {time[step[idx][0]]:.2f} s', linewidth=1)
                ax.plot(time[start_pos:end_pos], convolution[start_pos:end_pos], linewidth=1)
                ax.plot(time[start_pos:end_pos], norm_der[start_pos:end_pos], linewidth=1)
        ...
        figure_number = i // 3
        #fig.show()
        save_path = folder_name + '/' + f'{file_name}_flushes_{figure_number}.png'
        fig.savefig(save_path, bbox_inches='tight', pad_inches=0.1)
